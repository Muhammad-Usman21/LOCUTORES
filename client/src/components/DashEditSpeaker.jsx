...
const DashEditSpeaker = ({ stripeAccountId }) => {
	...
	const handleSubmit = async (e) => {
		...
		if (
			...
			setLoading(false);
			setSpeakerErrorMsg(
				"Solo el enlace de video de Youtube y Acerca de son opcionales.<br />Todos los demás campos son obligatorios."
			);
			return;
		}
		console.log(formData);
		try {
			...
			if (!res.ok) {
				...
				setLoading(false);
				setSpeakerErrorMsg(null);
				setUpdateMsg("Cuenta de oradora actualizada con éxito!");
		...
		<div
			...
			bg-transparent border-2 border-white/40 dark:border-white/20 backdrop-blur-[20px] rounded-lg shadow-xl">
				...
				<form
					...
					<div className="bg-transparent border-2 border-white/20 backdrop-blur-[20px] rounded-lg shadow-lg p-3 flex flex-col gap-2  dark:shadow-whiteLg">
